...
set SKEW 1.25  ;# Adjusted to 2.5% of clock period (faster)
set MINRISE 0.25 ;# Adjusted to 0.5% of clock period (faster)
set MAXRISE 1.25 ;# Adjusted to 2.5% of clock period (faster)
set MINFALL 0.25 ;# Adjusted to 0.5% of clock period (faster)
set MAXFALL 1.25 ;# Adjusted to 2.5% of clock period (faster)
...
set_clock_latency -rise -source 1.25 -early -late $EXTCLK  ;# Adjusted to 2.5% of clock period (faster)
set_clock_latency -fall -source 1.25 -early -late $EXTCLK  ;# Adjusted to 2.5% of clock period (faster)
# Define waveform settings for Network Latency
set_clock_latency -rise 1.25 $EXTCLK  ;# Adjusted to 2.5% of clock period (faster)
set_clock_latency -fall 1.25 $EXTCLK  ;# Adjusted to 2.5% of clock period (faster)

# CLOCK UNCERTAINTY
set_clock_uncertainty -setup 1.25 [get_clocks $EXTCLK]  ;# Adjusted to 2.5% of clock period (faster)
set_clock_uncertainty -hold  0.25 [get_clocks $EXTCLK]  ;# Adjusted to 0.5% of clock period (faster)

# Input delay definition: This is the delay coming from outside the design
# for this design it's defined at 5% the period of the clock (faster).
set_input_delay -clock [get_clocks $EXTCLK] -add_delay 2.5 -name I_DELAY  [get_ports -filter "(direction == in || direction == inout) && name != sysclk"]

# Output delay definition: This is the delay going outside the design
# for this design it's defined at 5% the period of the clock (faster).
set_output_delay -clock [get_clocks $EXTCLK] -add_delay 2.5 -name O_DELAY [get_ports -filter "direction == out || direction == inout"]
...
set_max_transition 10  ;# Faster transition
